...
export default function Layout({ children, onLogout }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-zinc-950">
      {/* Sidebar */}
      <nav className="hidden md:flex w-60 flex-col border-r border-zinc-800 bg-zinc-900">
        ...
        <button
          onClick={onLogout}
          className="flex items-center gap-3 mx-2 mb-4 px-3 py-2 rounded-lg text-sm text-zinc-400 hover:text-red-400 hover:bg-zinc-800/50 transition-colors"
        >
          <LogOut size={18} />
          {t("nav.logout")}
        </button>
            ...
            {navItems.slice(0, 4).map(({ path, icon: Icon }) => {
              ...
              return (
                <Link
                  key={path}
                  to={path}
                  className={`p-2 rounded-lg ${active ? "bg-zinc-800" : "text-zinc-400"}`}
                >
                  <Icon size={18} />
                </Link>
              );
