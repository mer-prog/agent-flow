# AgentFlow — Multi-Agent AI Customer Support Platform

A full-stack AI customer support system integrating LangGraph multi-agent orchestration, FastMCP tool integration, and RAG-based knowledge retrieval. Supports dual-mode operation: a demo mode requiring no API keys and a live mode powered by Claude Haiku 4.5 + OpenAI Embeddings.

## Skills Demonstrated

- **Full-Stack Development** — End-to-end development with React 19 SPA + async FastAPI backend
- **AI/LLM Integration** — LangGraph multi-agent graph, Claude Haiku 4.5, RAG pipeline, FastMCP tool integration
- **API Design & Integration** — RESTful API design (7 routers), SSE streaming, Model Context Protocol integration
- **Database Design & Management** — PostgreSQL 17 + pgvector for vector search, async ORM, migration management
- **Authentication & Authorization** — JWT authentication (access/refresh tokens), role-based access control (RBAC)
- **System Architecture** — State machine-based orchestration with 5 agents + formatter
- **UI/UX Design** — Tailwind CSS 4 dark theme, i18n multilingual support (EN/JP), real-time streaming UI
- **Testing & Quality Assurance** — 54 test cases covering agent logic, authentication, and schema validation

## Tech Stack

### Backend

| Category | Technology |
|----------|-----------|
| Framework | FastAPI 0.115+ |
| Agents | LangGraph 0.2+, LangChain Core 0.3+ |
| LLM | Claude Haiku 4.5 (langchain-anthropic 0.3+) |
| MCP | FastMCP 2.0+ |
| Database | PostgreSQL 17 + pgvector 0.3+ |
| ORM | SQLAlchemy 2.0 (async) + asyncpg 0.30+ |
| Migrations | Alembic 1.14+ |
| Embeddings | OpenAI text-embedding-3-small / SHA-256 (demo) |
| Auth | python-jose (JWT) + bcrypt 4.2+ |
| Logging | structlog 24.4+ |
| HTTP Client | httpx 0.28+ |

### Frontend

| Category | Technology |
|----------|-----------|
| Framework | React 19.0.0 |
| Routing | React Router 7.1+ |
| Styling | Tailwind CSS 4.0 |
| Build Tool | Vite 6.0 |
| Language | TypeScript 5.7+ |
| i18n | i18next 24.2+ (EN/JP) |
| Charts | Recharts 2.15+ |
| Icons | Lucide React 0.468+ |

### Infrastructure

| Category | Technology |
|----------|-----------|
| Container | Docker multi-stage build |
| Database | pgvector/pgvector:pg17 |
| Deployment | Render (free tier compatible) |
| Python Runtime | 3.12+ |
| Package Manager | uv |

## Architecture Overview

```text
┌─────────────────────────────────────────────────────────────────────┐
│                      React 19 Frontend                              │
│             Vite + TypeScript + Tailwind CSS 4 + i18next           │
│                                                                     │
│  Chat │ Tickets │ Escalations │ Knowledge │ Dashboard │ AgentFlow  │
└───────────────────────────┬─────────────────────────────────────────┘
                            │ HTTP / SSE
                            ▼
┌───────────────────────────────────────────────────────────────────────┐
│                         FastAPI Backend                               │
│                                                                       │
│  ┌─────────────────────────────────────────────────────────────────┐  │
│  │  API Routers (7)                                                │  │
│  │  auth │ chat │ conversations │ tickets │ escalations │          │  │
│  │  knowledge │ stats                                              │  │
│  └───────────────────────────┬─────────────────────────────────────┘  │
│                              │                                        │
│  ┌───────────────────────────▼─────────────────────────────────────┐  │
│  │            LangGraph Multi-Agent Graph                          │  │
│  │                                                                 │  │
│  │  router_agent ──┬──→ faq_agent ────────┐                       │  │
│  │  (intent        ├──→ ticket_agent ─────┤                       │  │
│  │   classifier)   ├──→ escalation_agent ──┤──→ formatter → END   │  │
│  │                 └──→ chitchat_agent ────┘   (response           │  │
│  │                                              formatting)        │  │
│  │  Live Mode: Claude Haiku 4.5 + OpenAI Embeddings               │  │
│  │  Demo Mode: Keyword classifiers + SHA-256 pseudo-embeddings    │  │
│  └─────────────────────────────────────────────────────────────────┘  │
│                                                                       │
│  ┌─────────────────────────────────────────────────────────────────┐  │
│  │  FastMCP Tool Server (5 Tools)                                  │  │
│  │  search_knowledge_base │ create_ticket │ update_ticket │        │  │
│  │  get_support_metrics │ escalate_to_human                        │  │
│  └─────────────────────────────────────────────────────────────────┘  │
│                                                                       │
│  ┌─────────────────────────────────────────────────────────────────┐  │
│  │  Service Layer                                                  │  │
│  │  auth │ embedding │ knowledge │ security                        │  │
│  └───────────────────────────┬─────────────────────────────────────┘  │
└──────────────────────────────┼────────────────────────────────────────┘
                               │
              ┌────────────────┴─────────────────┐
              ▼                                  ▼
  ┌─────────────────────┐           ┌──────────────────────┐
  │  PostgreSQL 17      │           │  External APIs       │
  │  + pgvector         │           │  Anthropic (Claude)  │
  │                     │           │  OpenAI (Embeddings) │
  │  8 Tables:          │           └──────────────────────┘
  │  users              │
  │  conversations      │
  │  messages           │
  │  tickets            │
  │  escalations        │
  │  agent_runs         │
  │  kb_articles        │
  │  kb_chunks          │
  └─────────────────────┘
```

## Key Features

### Multi-Agent Chat System

- **Intent Classification Router** — Automatically classifies user messages into 4 categories: FAQ, Ticket, Escalation, and Chitchat
- **RAG-Based FAQ Responses** — Retrieves relevant information from the knowledge base via pgvector similarity search to generate answers
- **Automatic Ticket Creation** — Extracts title, priority, and category from conversation content to auto-generate support tickets
- **Sentiment-Based Escalation** — Detects negative sentiment and suggests escalation to human operators
- **SSE Real-Time Streaming** — Token-by-token real-time response display

### Dual-Mode Operation

- **Live Mode** — High-accuracy NLP via Claude Haiku 4.5 + semantic search via OpenAI text-embedding-3-small
- **Demo Mode** — No API keys required. Keyword-based classifiers + SHA-256 pseudo-embeddings enable full functionality

### Knowledge Base Management

- Article CRUD operations
- Automatic chunking and embedding generation
- Cosine similarity vector search
- Category filtering and publication status management

### Ticket Management

- 4-level priority (low, medium, high, urgent)
- Status tracking (open, in_progress, resolved, closed)
- Role-based display control (customers see own tickets; agents/admins see all)

### Escalation Management

- Pending escalations queue
- Sentiment score-based prioritization
- Approval/rejection workflow

### Dashboard & Analytics

- Conversation, ticket, and escalation statistics
- Agent performance metrics
- Resolution rate and response time visualization (Recharts)

### Authentication & Authorization

- JWT tokens (access + refresh)
- 3 user roles: admin, agent, customer
- Automatic token refresh on 401

### Multilingual Support

- EN/JP language switching via i18next
- Automatic browser language detection

## Project Structure

```text
agent-flow/
├── app/                          # FastAPI backend
│   ├── main.py                  # Entry point, middleware config
│   ├── config.py                # Settings (DB, JWT, LLM keys)
│   ├── database.py              # SQLAlchemy async engine & session
│   ├── models/                  # SQLAlchemy ORM models (8 tables)
│   │   ├── user.py             # User, UserRole enum
│   │   ├── conversation.py     # Conversation, ConversationStatus
│   │   ├── message.py          # Message, MessageRole
│   │   ├── ticket.py           # Ticket, TicketStatus, TicketPriority
│   │   ├── escalation.py       # Escalation, EscalationStatus
│   │   ├── agent_run.py        # AgentRun, AgentType
│   │   └── knowledge.py        # KBArticle, KBChunk (pgvector)
│   ├── schemas/                 # Pydantic request/response schemas
│   │   ├── auth.py             # Auth schemas
│   │   ├── chat.py             # Chat schemas
│   │   ├── conversation.py     # Conversation schemas
│   │   ├── ticket.py           # Ticket schemas
│   │   ├── escalation.py       # Escalation schemas
│   │   ├── knowledge.py        # Knowledge base schemas
│   │   └── stats.py            # Statistics schemas
│   ├── api/                     # REST API routers
│   │   ├── auth.py             # /api/auth
│   │   ├── chat.py             # /api/chat (SSE streaming)
│   │   ├── conversations.py    # /api/conversations
│   │   ├── tickets.py          # /api/tickets
│   │   ├── escalations.py      # /api/escalations
│   │   ├── knowledge.py        # /api/knowledge
│   │   ├── stats.py            # /api/stats
│   │   └── deps.py             # Dependency injection (auth, DB)
│   ├── agents/                  # LangGraph agent system
│   │   ├── state.py            # AgentState type definition
│   │   ├── graph.py            # Agent graph build & compile
│   │   ├── llm.py              # LLM client singleton
│   │   ├── router.py           # Router agent (intent classification)
│   │   ├── faq.py              # FAQ agent (RAG search)
│   │   ├── ticket.py           # Ticket agent (field extraction)
│   │   ├── escalation.py       # Escalation agent (sentiment analysis)
│   │   ├── chitchat.py         # Chitchat agent
│   │   └── formatter.py        # Response formatter
│   ├── mcp/                     # FastMCP tool server
│   │   └── server.py           # 5 MCP tool definitions
│   ├── services/                # Business logic
│   │   ├── auth.py             # Auth helpers
│   │   ├── embedding.py        # Embedding service (live/demo toggle)
│   │   └── knowledge.py        # Knowledge base search & chunking
│   └── core/                    # Core utilities
│       ├── security.py         # JWT/bcrypt
│       └── logging.py          # structlog configuration
├── frontend/                     # React 19 SPA
│   ├── src/
│   │   ├── App.tsx             # Routing & auth wrapper
│   │   ├── api/client.ts       # HTTP client (auto token refresh)
│   │   ├── hooks/              # Custom hooks (useAuth, useSSE)
│   │   ├── i18n/               # Internationalization (en.ts, ja.ts)
│   │   ├── pages/              # Page components (8 pages)
│   │   ├── components/         # Shared components
│   │   └── types/              # TypeScript type definitions
│   └── package.json
├── alembic/                      # Database migrations
│   └── versions/
│       └── 001_initial.py      # All 8 tables + pgvector extension
├── tests/                        # Test suite (54 tests)
│   ├── test_agents.py          # Agent logic
│   ├── test_config.py          # Configuration validation
│   ├── test_embedding.py       # Embedding generation
│   ├── test_schemas.py         # Schema validation
│   └── test_security.py        # JWT/bcrypt
├── docker-compose.yml            # PostgreSQL 17 + pgvector
├── Dockerfile                    # Multi-stage build
├── render.yaml                   # Render deployment config
├── start.sh                      # Startup script
└── pyproject.toml                # Python dependencies & config
```

## API Endpoints

### Authentication (/api/auth)

| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/auth/register` | Register a new user |
| POST | `/api/auth/login` | Login (issue tokens) |
| POST | `/api/auth/refresh` | Refresh access token |
| GET | `/api/auth/me` | Get current user profile |

### Chat (/api/chat)

| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/chat` | Send message (SSE streaming response) |
| GET | `/api/chat/{conversation_id}/history` | Get conversation history |

### Conversations (/api/conversations)

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/conversations` | List conversations (paginated) |
| GET | `/api/conversations/{id}` | Get conversation detail (with messages) |
| PATCH | `/api/conversations/{id}` | Update conversation (title/status) |
| DELETE | `/api/conversations/{id}` | Delete conversation |

### Tickets (/api/tickets)

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/tickets` | List tickets (role-aware display) |
| POST | `/api/tickets` | Create ticket |
| GET | `/api/tickets/{id}` | Get ticket details |
| PATCH | `/api/tickets/{id}` | Update ticket |

### Escalations (/api/escalations)

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/escalations` | List escalations (admins/agents only) |
| GET | `/api/escalations/{id}` | Get escalation details |
| POST | `/api/escalations/{id}/review` | Review escalation (approve/reject) |

### Knowledge Base (/api/knowledge)

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/knowledge` | List published articles |
| POST | `/api/knowledge` | Create article (admins/agents) |
| GET | `/api/knowledge/{id}` | Get article detail (with chunks) |
| PATCH | `/api/knowledge/{id}` | Update article (re-chunks on content change) |
| DELETE | `/api/knowledge/{id}` | Delete article |
| POST | `/api/knowledge/search` | Vector similarity search |

### Statistics (/api/stats)

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/stats` | Dashboard statistics |
| GET | `/api/stats/agent-performance` | Agent performance metrics |

### Health Check

| Method | Path | Description |
|--------|------|-------------|
| GET | `/health` | Health check (shows operation mode) |

## Database Schema

### Table Definitions

| Table | Key Columns | Description |
|-------|-------------|-------------|
| **users** | id (UUID), email, hashed_password, full_name, role, is_active | User accounts |
| **conversations** | id (UUID), user_id (FK), title, status, metadata (JSONB) | Chat conversations |
| **messages** | id (UUID), conversation_id (FK), role, content, metadata (JSONB) | Messages within conversations |
| **tickets** | id (UUID), conversation_id (FK), user_id (FK), title, description, status, priority, category, assigned_to (FK) | Support tickets |
| **escalations** | id (UUID), conversation_id (FK), ticket_id (FK), reason, sentiment_score, status, reviewed_by (FK), notes | Escalation records |
| **agent_runs** | id (UUID), conversation_id (FK), agent_type, input_data (JSONB), output_data (JSONB), duration_ms, status | Agent execution logs |
| **kb_articles** | id (UUID), title, content, category, is_published | Knowledge base articles |
| **kb_chunks** | id (UUID), article_id (FK), content, embedding (Vector(1536)), chunk_index | Embedding chunks |

### Enums

| Enum | Values |
|------|--------|
| UserRole | admin, agent, customer |
| ConversationStatus | active, closed, archived |
| MessageRole | user, assistant, system |
| TicketStatus | open, in_progress, resolved, closed |
| TicketPriority | low, medium, high, urgent |
| EscalationStatus | pending, approved, rejected, completed |
| AgentType | router, faq, ticket, escalation, chitchat, formatter |
| AgentRunStatus | started, completed, failed |

## AI/ML Pipeline

### Agent Graph Processing Flow

```text
User Message
       │
       ▼
  router_agent
  (Intent Classification: FAQ / Ticket / Escalation / Chitchat)
       │
       ├─── intent: faq ──────→ faq_agent
       │                        ├─ KB vector search (pgvector)
       │                        └─ Generate answer from search results
       │
       ├─── intent: ticket ───→ ticket_agent
       │                        ├─ Extract title, priority, category
       │                        └─ Auto-create support ticket
       │
       ├─── intent: escalation → escalation_agent
       │                         ├─ Calculate sentiment score (0.0-1.0)
       │                         └─ Set human review flag
       │
       └─── intent: chitchat ──→ chitchat_agent
                                 └─ General conversational response
       │
       ▼
   formatter
   (Final response formatting with metadata)
       │
       ▼
   Return response via SSE streaming
```

### Dual-Mode Implementation

| Component | Live Mode | Demo Mode |
|-----------|-----------|-----------|
| Intent Classification | Claude Haiku 4.5 | Regex pattern matching |
| FAQ Responses | Claude Haiku 4.5 + KB search | Template responses |
| Ticket Extraction | Claude Haiku 4.5 | Regex field extraction |
| Sentiment Analysis | Claude Haiku 4.5 (0.0-1.0) | Keyword-based analysis |
| Chitchat Responses | Claude Haiku 4.5 | Template responses |
| Embedding Generation | OpenAI text-embedding-3-small | SHA-256 pseudo-embeddings |

### MCP Tool Integration

Via FastMCP 2.0, agents can invoke the following tools:

- **search_knowledge_base_tool** — Knowledge base search using pgvector cosine similarity
- **create_ticket** — Create support tickets
- **update_ticket** — Update ticket status and priority
- **get_support_metrics** — Aggregate dashboard statistics
- **escalate_to_human** — Create escalation to human operators

## Security

### Authentication

- JWT tokens (HS256 algorithm)
- Access token: 30-minute validity
- Refresh token: 7-day validity
- Password hashing with bcrypt

### Authorization

- Role-based access control (admin, agent, customer)
- Per-endpoint permission checks
- Customer data isolation (access only own resources)

### API Security

- CORS middleware (configurable allowed origins)
- Request validation via Pydantic schemas
- Email format and minimum password length (8 chars) validation

## Setup

### Prerequisites

- Python 3.12+
- Node.js 20+
- Docker (for PostgreSQL)
- uv (Python package manager)

### Installation

```bash
# 1. Start PostgreSQL + pgvector
docker compose up -d

# 2. Configure environment variables
cp .env.example .env
# Edit .env (make sure to change JWT_SECRET)

# 3. Install Python dependencies
uv sync

# 4. Run database migrations
alembic upgrade head

# 5. Start the backend
uv run fastapi dev app/main.py

# 6. Start the frontend (separate terminal)
cd frontend && npm install && npm run dev
```

### Environment Variables

| Variable | Required | Description | Default |
|----------|----------|-------------|---------|
| `DATABASE_URL` | Yes | PostgreSQL connection string | — |
| `JWT_SECRET` | Yes | JWT signing secret key | — |
| `ANTHROPIC_API_KEY` | No | Anthropic API key (for live mode) | — |
| `OPENAI_API_KEY` | No | OpenAI API key (for live mode) | — |
| `APP_ENV` | No | Runtime environment | `development` |
| `LOG_LEVEL` | No | Log level | `INFO` |
| `CORS_ORIGINS` | No | CORS allowed origins (JSON array) | `["http://localhost:5173"]` |

### Docker Build (Production)

```bash
docker build -t agent-flow .
docker run -p 8000:8000 --env-file .env agent-flow
```

## Testing

```bash
# Run all tests
uv run pytest

# Stop on first failure
uv run pytest -x

# Verbose output
uv run pytest -v
```

### Test Coverage

| Test File | Tests | Coverage |
|-----------|-------|----------|
| test_agents.py | 24 | Agent intent classification, sentiment analysis, ticket extraction |
| test_schemas.py | 14 | Pydantic schema validation |
| test_security.py | 8 | JWT token generation/verification, password hashing |
| test_config.py | 4 | Configuration loading, default values |
| test_embedding.py | 4 | Embedding generation, dimensionality |

## Design Decisions

### LangGraph Multi-Agent Orchestration

Implemented a 3-stage pipeline (intent classification → specialist agent → formatter) as a state machine. Each agent operates independently and coordinates through shared state (AgentState TypedDict), making it easy to add new agents or modify existing ones.

### Dual-Mode (Live/Demo)

Automatically switches modes based on API key availability. Developers can trial all features without API costs, then seamlessly transition to high-accuracy mode in production. The demo mode was designed to fully demonstrate the entire application flow.

### FastMCP Tool Integration

Standardized agent tools (knowledge search, ticket operations, etc.) via the MCP protocol. This achieves decoupling where adding or modifying tools does not impact agent code.

### pgvector for Vector Search

Leveraged PostgreSQL's pgvector extension instead of introducing a dedicated vector database. This minimizes infrastructure complexity while enabling cosine similarity-based semantic search.

### SSE Streaming

Chose Server-Sent Events over WebSocket. For requirements where unidirectional streaming (server → client) is sufficient, this prioritizes implementation simplicity and HTTP/2 compatibility.

### Async-First

All database operations, API handlers, and agent nodes use async/await. This optimizes concurrent processing of I/O-bound LLM API calls and database queries.

## Running Costs

### Free Tier Operation

| Service | Plan | Monthly Cost |
|---------|------|-------------|
| Render Web Service | Free | $0 |
| Render PostgreSQL | Free | $0 |
| **Total (Demo Mode)** | | **$0** |

### Additional Costs with Live Mode (API Keys)

| Service | Pricing | Estimated Cost |
|---------|---------|---------------|
| Anthropic Claude Haiku 4.5 | Input $0.80/1M tokens, Output $4.00/1M tokens | ~$1-5/month (light usage) |
| OpenAI Embeddings (text-embedding-3-small) | $0.02/1M tokens | ~$0.01-0.10/month |
| **Total (Live Mode)** | | **~$1-5/month** |

> Demo mode requires no API keys and operates completely free of charge.

## Author

**@mer-prog**

GitHub: [https://github.com/mer-prog](https://github.com/mer-prog)
